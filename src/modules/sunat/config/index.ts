...
export function config(): Config {
  const state = join(REPO_ROOT, "state", "sunat_ingest");
  mkdirSync(state, { recursive: true });
  return {
    concurrency: env.get(ENV.SUNAT_CONCURRENCY).default("2").asIntPositive(),
    minDelay: env.get(ENV.SUNAT_DELAY).default("0.4").asFloat(),
    limit: env.get(ENV.SUNAT_LIMIT).default("0").asInt() || null,
    maxRetries: MAX_RETRIES,
    backoffBase: BACKOFF_BASE,
    requestTimeout: REQUEST_TIMEOUT,
    progressEvery: PROGRESS_EVERY,
    userAgent: env.get(ENV.SUNAT_UA).default(DEFAULT_USER_AGENT).asString(),
    anioDesde: env.get(ENV.SUNAT_ANIO_DESDE).default(String(PRIMER_ANIO)).asIntPositive(),
    anioHasta: env.get(ENV.SUNAT_ANIO_HASTA).default("0").asInt(),
    ...
    ingestSource: env
      ...
      .default(sourceByKey("sunat").canonicalName)
      .asString(),
