...
/**
 * Emisor FIJO: SERVIR (dueña del Tribunal del Servicio Civil). Primero por
 * sigla única del catálogo y si no, por nombre completo. Falla ruidosamente
 * si faltara.
 */
function resolveIssuer(idx: Index): Classif {
  const porSigla = classifier.entityByAcronym(idx, "SERVIR");
  if (porSigla) return { ...porSigla, match_confidence: "exact" };
  const nombre = "Autoridad Nacional del Servicio Civil";
  const entity = idx.exact[classifier.normalize(nombre)];
  if (!entity) {
    throw new Error("El catálogo public/data/entity.json no tiene la entidad SERVIR.");
  }
  const clasif = classifier.classifFromEntityId(idx, entity.id, "exact");
  if (!clasif) {
    throw new Error("No se pudo construir la clasificación del emisor SERVIR.");
  }
  return clasif;
}
...
function summary(
  ...
  log.info("=".repeat(60));
  log.info("RESUMEN FINAL (ingesta Tribunal del Servicio Civil)");
