...
export interface Config {
  ...
  userAgent: string;
  /** Término del buscador (default "TSC"). */
  term: string;
...
/** Una resolución del Tribunal del Servicio Civil publicada en gob.pe. */
export interface Doc {
  /** Id numérico de gob.pe (del href) — id del ledger. */
  gid: string;
  /** Número limpio ("005982-2024-Servir/TSC:Primera Sala"). */
  numero: string;
  sumilla: string;
...
export interface Ctx {
  ...
  log: Logger;
  idx: Index;
  /** Emisor FIJO del módulo: la entidad SERVIR del catálogo. */
  issuer: Classif;
  stats: Stats;
