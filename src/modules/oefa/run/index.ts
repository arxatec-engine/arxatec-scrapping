...
/**
 * Emisor FIJO: OEFA (dueño del Tribunal de Fiscalización Ambiental). Primero por
 * sigla única del catálogo y si no, por nombre completo. Falla ruidosamente
 * si faltara.
 */
function resolveIssuer(idx: Index): Classif {
  const porSigla = classifier.entityByAcronym(idx, "OEFA");
  if (porSigla) return { ...porSigla, match_confidence: "exact" };
  const nombre = "Organismo de Evaluación y Fiscalización Ambiental";
  const entity = idx.exact[classifier.normalize(nombre)];
  if (!entity) {
    throw new Error("El catálogo public/data/entity.json no tiene la entidad OEFA.");
  }
  const clasif = classifier.classifFromEntityId(idx, entity.id, "exact");
  if (!clasif) {
    throw new Error("No se pudo construir la clasificación del emisor OEFA.");
  }
  return clasif;
}
...
export async function run(
  ...
  // Sin checkpoint de cursor: cada corrida barre el buscador (el stream con término; SUNARP_MAX_SHEETS acota en pruebas,
...
function summary(
  ...
  log.info("=".repeat(60));
  log.info("RESUMEN FINAL (ingesta Tribunal de Fiscalización Ambiental)");
