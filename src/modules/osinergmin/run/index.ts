...
import { ENTITY_NAME, SIGLA } from "../constants";

/**
 * Emisor FIJO: OSINERGMIN. Primero por sigla única del catálogo y si no, por
 * nombre completo. Falla ruidosamente si faltara.
 */
function resolveIssuer(idx: Index): Classif {
  const porSigla = classifier.entityByAcronym(idx, SIGLA);
  if (porSigla) return { ...porSigla, match_confidence: "exact" };
  const entity = idx.exact[classifier.normalize(ENTITY_NAME)];
  if (!entity) {
    throw new Error(`El catálogo public/data/entity.json no tiene la entidad ${SIGLA}.`);
  }
  const clasif = classifier.classifFromEntityId(idx, entity.id, "exact");
  if (!clasif) {
    throw new Error(`No se pudo construir la clasificación del emisor ${SIGLA}.`);
  }
  return clasif;
}
...
export async function run(
  ...
  // Sin checkpoint de cursor: cada corrida barre el buscador (el stream completo del regulador,
...
function summary(
  ...
  log.info("=".repeat(60));
  log.info("RESUMEN FINAL (ingesta OSINERGMIN)");
