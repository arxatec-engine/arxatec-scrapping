...
import { fetchVentana } from "../services/adlp";
...
/** Emisor FIJO: Congreso de la República (mismo patrón que spley). */
function resolveIssuer(idx: Index): Classif {
  const entity = idx.exact[classifier.normalize("Congreso de la República")];
  if (!entity) {
    throw new Error(
      "El catálogo public/data/entity.json no tiene la entidad Congreso de la República."
    );
  }
...
export async function run(cfg: Config, log: Logger): Promise<void> {
  ...
  const ctx: Ctx = {
    ...
    adlpThrottle: newThrottle(cfg.minDelay),
    browser,
    form: null,
  };
  ingest.prepare(ctx);

  // Ventanas de números reciente-primero (≤ VENTANA por consulta: el grid
  // trunca a 20 filas sin paginador). Sin checkpoint: el ledger es la memoria.
  const vistos = new Set<string>();
  let derogadasVistas = 0;
  const sem = semaphore(cfg.concurrency);
  let topeAlcanzado = false;
  try {
    let nuevos = 0;

    for (let fin = cfg.numHasta; fin >= cfg.numDesde; fin -= VENTANA) {
      const ini = Math.max(cfg.numDesde, fin - VENTANA + 1);
      const docs = await fetchVentana(ctx, ini, fin);
      if (docs.length === 0) {
        log.info("Ventana %d-%d sin normas.", ini, fin);
        continue;
      }
      for (const d of docs) {
        vistos.add(d.id);
        if (d.derogada) derogadasVistas += 1;
      }
      const pendientes = docs.filter((d) => !processed.has(d.id));
      log.info(
        "Ventana %d-%d: %d normas (%d derogadas), %d por procesar.",
        ini,
        fin,
        docs.length,
        docs.filter((d) => d.derogada).length,
        pendientes.length
      );
  ...
  log.info("Derogadas vistas en los índices: %d.", derogadasVistas);
...
function summary(
  ...
  log.info("=".repeat(60));
  log.info("RESUMEN FINAL (ingesta ADLP — normas con rango de ley)");
  log.info("  Normas vistas en los índices (esta corrida): %d", vistos);
  ...
  if (pendientes === 0 && stats.procesados === 0 && !parcial) {
    log.info("✓ NADA NUEVO: los índices no trajeron normas sin ingestar.");
