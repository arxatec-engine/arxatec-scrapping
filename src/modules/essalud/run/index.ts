...
/**
 * Emisor FIJO: ESSALUD. Primero por sigla única del catálogo y si no, por
 * nombre completo. Falla ruidosamente si faltara.
 */
function resolveIssuer(idx: Index): Classif {
  const porSigla = classifier.entityByAcronym(idx, "ESSALUD");
  if (porSigla) return { ...porSigla, match_confidence: "exact" };
  const nombre = "Seguro Social de Salud";
  const entity = idx.exact[classifier.normalize(nombre)];
  if (!entity) {
    throw new Error("El catálogo public/data/entity.json no tiene la entidad ESSALUD.");
  }
  const clasif = classifier.classifFromEntityId(idx, entity.id, "exact");
  if (!clasif) {
    throw new Error("No se pudo construir la clasificación del emisor ESSALUD.");
  }
  return clasif;
}
...
export async function run(
  ...
  // Sin checkpoint de cursor: cada corrida barre el buscador (el stream completo; ESSALUD_MAX_SHEETS acota en pruebas,
...
function summary(
  ...
  log.info("=".repeat(60));
  log.info("RESUMEN FINAL (ingesta ESSALUD — normativa)");
