...
export function config(): Config {
  const state = join(REPO_ROOT, "state", "essalud_ingest");
  mkdirSync(state, { recursive: true });
  return {
    concurrency: env.get(ENV.ESSALUD_CONCURRENCY).default("2").asIntPositive(),
    minDelay: env.get(ENV.ESSALUD_DELAY).default("0.4").asFloat(),
    limit: env.get(ENV.ESSALUD_LIMIT).default("0").asInt() || null,
    maxRetries: SEARCH_MAX_RETRIES,
    backoffBase: BACKOFF_BASE,
    progressEvery: PROGRESS_EVERY,
    userAgent: env.get(ENV.ESSALUD_UA).default(DEFAULT_USER_AGENT).asString(),
    term: env.get(ENV.ESSALUD_TERM).default(DEFAULT_TERM).asString(),
    maxSheets: env.get(ENV.ESSALUD_MAX_SHEETS).default("0").asInt(),
    ...
    ingestSource: env
      ...
      .default(sourceByKey("essalud").canonicalName)
      .asString(),
