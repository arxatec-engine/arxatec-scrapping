...
export function config(): Config {
  const state = join(REPO_ROOT, "state", "sunarp_ingest");
  mkdirSync(state, { recursive: true });
  return {
    concurrency: env.get(ENV.SUNARP_CONCURRENCY).default("2").asIntPositive(),
    minDelay: env.get(ENV.SUNARP_DELAY).default("0.4").asFloat(),
    limit: env.get(ENV.SUNARP_LIMIT).default("0").asInt() || null,
    maxRetries: SEARCH_MAX_RETRIES,
    backoffBase: BACKOFF_BASE,
    progressEvery: PROGRESS_EVERY,
    userAgent: env.get(ENV.SUNARP_UA).default(DEFAULT_USER_AGENT).asString(),
    term: env.get(ENV.SUNARP_TERM).default(DEFAULT_TERM).asString(),
    maxSheets: env.get(ENV.SUNARP_MAX_SHEETS).default("0").asInt(),
    ...
    ingestSource: env
      ...
      .default(sourceByKey("tregistral").canonicalName)
      .asString(),
