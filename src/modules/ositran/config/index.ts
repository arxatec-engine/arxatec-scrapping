...
export function config(): Config {
  const state = join(REPO_ROOT, "state", "ositran_ingest");
  mkdirSync(state, { recursive: true });
  return {
    concurrency: env.get(ENV.OSITRAN_CONCURRENCY).default("2").asIntPositive(),
    minDelay: env.get(ENV.OSITRAN_DELAY).default("0.4").asFloat(),
    limit: env.get(ENV.OSITRAN_LIMIT).default("0").asInt() || null,
    maxRetries: SEARCH_MAX_RETRIES,
    backoffBase: BACKOFF_BASE,
    progressEvery: PROGRESS_EVERY,
    userAgent: env.get(ENV.OSITRAN_UA).default(DEFAULT_USER_AGENT).asString(),
    maxSheets: env.get(ENV.OSITRAN_MAX_SHEETS).default("0").asInt(),
    ...
    ingestSource: env
      ...
      .default(sourceByKey("ositran").canonicalName)
      .asString(),
