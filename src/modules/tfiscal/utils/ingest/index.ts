...
export async function ingestOne(ctx: Ctx, doc: Doc): Promise<void> {
  ...
  const filename = `${sanitize(doc.rtf, 60)}.pdf`;
  ...
  try {
    ...
    // Área por IA sobre la sumilla (la mayoría caerá en tributario); sin
    // sumilla el análisis viene vacío y el área queda en el default CON warning.
    ...
    // con OCR local y se reingesta un PDF de texto renderizado; el escaneo
    // original sigue enlazado por source_url.
    if (
      !result.ok &&
      result.permanent &&
      /extractable text/i.test(result.error ?? "")
    ) {
      log.info("RTF %s: PDF escaneado; intento OCR local…", doc.rtf);
      const texto = await ocrPdf(pdfBytes, log);
      if (texto) {
        ocrUsado = true;
        const html =
          `<p><em>Texto extraído por OCR del escaneo original ` +
          `(RTF ${doc.rtf}).</em></p>` +
    ...
    log.warn("RTF %s: fallo preparando/enviando ingesta: %s", doc.rtf, msg);
    record(ctx, doc, {
      ok: false,
      permanent: false,
      error: msg,
  ...
  if (result.ok) {
    stats.descargados += 1;
    const d = result.data;
    // QA post-ingesta (patrón A4). Para RTF viejas el PDF puede ser escaneado:
    // pages_with_text=0 queda marcado como cola de OCR futura.
    const problemas: string[] = [];
    if (!d.linked_entities) {
      problemas.push("emisor no enlazado: el backend no vinculó Tribunal Fiscal");
    }
    if (areaFallback) {
    ...
    if (problemas.length > 0) {
      ...
      log.warn("RTF %s: %s", doc.rtf, warning);
    }
    log.info(
      "Ingestado RTF %s -> doc=%s chunks=%s paginas=%s entidades=%s",
      doc.rtf,
      d.document_id,
      d.indexed_chunks,
      d.pages_with_text,
    ...
    stats.errores += 1;
    log.warn(
      "Ingesta RTF %s rechazada (status=%s, permanente=%s): %s",
      doc.rtf,
      result.status,
      result.permanent,
      result.error
