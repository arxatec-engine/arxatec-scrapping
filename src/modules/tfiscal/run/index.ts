...
/**
 * Emisor FIJO del módulo: la entidad "Tribunal Fiscal" del catálogo. Se
 * resuelve por nombre normalizado (no por uuid quemado) para sobrevivir a
 * refrescos del catálogo; si desapareciera, mejor reventar aquí que ingestar
 * miles de RTF sin emisor.
 */
function resolveIssuer(idx: Index): Classif {
  const entity = idx.exact[classifier.normalize("Tribunal Fiscal")];
  if (!entity) {
    throw new Error('El catálogo public/data/entity.json no tiene la entidad "Tribunal Fiscal".');
  }
  const clasif = classifier.classifFromEntityId(idx, entity.id, "exact");
  if (!clasif) {
    throw new Error("No se pudo construir la clasificación del emisor Tribunal Fiscal.");
  }
  return clasif;
}
...
export async function run(
  ...
  // Sin checkpoint de cursor: como en `entidades`, cada corrida barre el
  // buscador completo (≈310 páginas, minutos) y el LEDGER es la memoria — con
  // orden=recientes lo nuevo aparece al frente y lo viejo se salta barato.
...
function summary(
  ...
  log.info("=".repeat(60));
  log.info("RESUMEN FINAL (ingesta Tribunal Fiscal)");
  log.info("  RTF vistas en el buscador (esta corrida): %d", vistos);
  ...
  if (pendientes === 0 && stats.procesados === 0 && !parcial) {
    log.info("✓ NADA NUEVO: el buscador no trajo RTF sin ingestar.");
