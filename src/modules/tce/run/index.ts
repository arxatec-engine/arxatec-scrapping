...
/**
 * Emisor FIJO: el OECE (dueño actual del Tribunal; ex OSCE). Primero por
 * sigla única del catálogo y si no, por nombre completo. Falla ruidosamente
 * si faltara.
 */
function resolveIssuer(idx: Index): Classif {
  const porSigla = classifier.entityByAcronym(idx, "OECE");
  if (porSigla) return { ...porSigla, match_confidence: "exact" };
  const nombre = "Organismo Especializado para las Contrataciones Públicas Eficientes";
  const entity = idx.exact[classifier.normalize(nombre)];
  if (!entity) {
    throw new Error("El catálogo public/data/entity.json no tiene la entidad OECE/OSCE.");
  }
  const clasif = classifier.classifFromEntityId(idx, entity.id, "exact");
  if (!clasif) {
    throw new Error("No se pudo construir la clasificación del emisor OECE.");
  }
  return clasif;
}
...
export async function run(
  ...
  // Sin checkpoint de cursor: cada corrida barre el buscador (~3.430 páginas (el OECE publica 85.7k normas; TCE_MAX_SHEETS acota en pruebas),
...
function summary(
  ...
  log.info("=".repeat(60));
  log.info("RESUMEN FINAL (ingesta Tribunal de Contrataciones)");
