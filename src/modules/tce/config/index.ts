...
export function config(): Config {
  const state = join(REPO_ROOT, "state", "tce_ingest");
  mkdirSync(state, { recursive: true });
  return {
    concurrency: env.get(ENV.TCE_CONCURRENCY).default("2").asIntPositive(),
    minDelay: env.get(ENV.TCE_DELAY).default("0.4").asFloat(),
    limit: env.get(ENV.TCE_LIMIT).default("0").asInt() || null,
    maxRetries: SEARCH_MAX_RETRIES,
    backoffBase: BACKOFF_BASE,
    progressEvery: PROGRESS_EVERY,
    userAgent: env.get(ENV.TCE_UA).default(DEFAULT_USER_AGENT).asString(),
    maxSheets: env.get(ENV.TCE_MAX_SHEETS).default("0").asInt(),
    ...
    ingestSource: env
      ...
      .default(sourceByKey("tce").canonicalName)
      .asString(),
