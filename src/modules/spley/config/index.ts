...
export function config(): Config {
  const state = join(REPO_ROOT, "state", "spley_ingest");
  mkdirSync(state, { recursive: true });
  return {
    concurrency: env.get(ENV.SPLEY_CONCURRENCY).default("2").asIntPositive(),
    minDelay: env.get(ENV.SPLEY_DELAY).default("0.4").asFloat(),
    limit: env.get(ENV.SPLEY_LIMIT).default("0").asInt() || null,
    maxRetries: MAX_RETRIES,
    backoffBase: BACKOFF_BASE,
    requestTimeout: REQUEST_TIMEOUT,
    progressEvery: PROGRESS_EVERY,
    userAgent: env.get(ENV.SPLEY_UA).default(DEFAULT_USER_AGENT).asString(),
    periodos: env
      .get(ENV.SPLEY_PERIODOS)
      .default("")
      .asString()
      .split(",")
      .map((s) => Number(s.trim()))
    ...
    ingestSource: env
      ...
      .asString(),
    // Un proyecto NO es norma vigente: "En revisión" (valor del filtro).
    ingestStatus: env.get(ENV.INGEST_STATUS).default(STATUS_PROYECTO).asString(),
  };
}
