...
export function config(): Config {
  const state = join(REPO_ROOT, "state", "osiptel_ingest");
  mkdirSync(state, { recursive: true });
  return {
    concurrency: env.get(ENV.OSIPTEL_CONCURRENCY).default("2").asIntPositive(),
    minDelay: env.get(ENV.OSIPTEL_DELAY).default("0.4").asFloat(),
    limit: env.get(ENV.OSIPTEL_LIMIT).default("0").asInt() || null,
    maxRetries: SEARCH_MAX_RETRIES,
    backoffBase: BACKOFF_BASE,
    progressEvery: PROGRESS_EVERY,
    userAgent: env.get(ENV.OSIPTEL_UA).default(DEFAULT_USER_AGENT).asString(),
    maxSheets: env.get(ENV.OSIPTEL_MAX_SHEETS).default("0").asInt(),
    ...
    ingestSource: env
      ...
      .default(sourceByKey("osiptel").canonicalName)
      .asString(),
