...
/**
 * Emisor FIJO: la entidad INDECOPI del catálogo, resuelta por nombre
 * normalizado (el nombre completo oficial). Falla ruidosamente si faltara.
 */
function resolveIssuer(idx: Index): Classif {
  ...
  if (!entity) {
    throw new Error("El catálogo public/data/entity.json no tiene la entidad INDECOPI.");
  }
  const clasif = classifier.classifFromEntityId(idx, entity.id, "exact");
  if (!clasif) {
    throw new Error("No se pudo construir la clasificación del emisor INDECOPI.");
  }
  return clasif;
}
...
export async function run(cfg: Config, log: Logger): Promise<void> {
  ...
  // Sin checkpoint de cursor: cada corrida barre el buscador (~125 páginas,
...
function summary(
  ...
  log.info("=".repeat(60));
  log.info("RESUMEN FINAL (ingesta INDECOPI)");
