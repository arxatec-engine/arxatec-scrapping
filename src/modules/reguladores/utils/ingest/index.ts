...
function reguladorDe(ctx: Ctx, doc: Doc): ReguladorCtx {
  const reg = ctx.reguladores[doc.institucion];
  if (!reg) {
    throw new Error(`Regulador desconocido en el ledger: "${doc.institucion}"`);
  }
  return reg;
}
...
export async function ingestOne(ctx: Ctx, doc: Doc): Promise<void> {
  ...
  try {
    ...
    meta = buildMetadata(doc, reg, area, cfg, analisis.concepts, analisis.references);
    result = await ingestRequest(ctx, pdfBytes, filename, meta);

    // Fallback OCR compartido (patrón P3): escaneados → PDF de texto.
    if (
      !result.ok &&
      result.permanent &&
  ...
  if (result.ok) {
    ...
    if (!d.linked_entities) {
      problemas.push(`emisor no enlazado: el backend no vinculó ${reg.sigla}`);
    }
    if (areaFallback) {
...
export function record(
  ...
  bumpConf(ctx.stats, reg.issuer.match_confidence);
