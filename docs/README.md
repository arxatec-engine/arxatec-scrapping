...
POST al **mismo endpoint** de ingesta del backend `arxatec-lawyer-assistant`.
Cuántos módulos hay hoy y cuáles: [`registro-scraping.md`](./registro-scraping.md),
que es el tablero vivo. **No pongas el número aquí** — este párrafo dijo durante
semanas «hoy existe un módulo funcionando: SPIJ» cuando ya había más de treinta.
...
| 0b | [`arquitectura-produccion.md`](./arquitectura-produccion.md) | **Dónde y cómo se corre en producción (2026-08-03).** Railway vs. PC propia (gana la PC: IP residencial por el PJ, y 4-5× más barato), por qué 20 sesiones en paralelo no sirven (~16 GB y cero ganancia: el cuello es el backend), antibot real por sitio y requisitos de máquina. Con mediciones: **~800 MB de RAM por sesión con Chrome**. |
...
**El estado vive en un solo sitio: [`registro-scraping.md`](./registro-scraping.md)**,
el tablero que se actualiza al cerrar cada módulo. Aquí no se duplica.

Aquí había una sección «Estado actual (2026-07-21)» que se retiró el 2026-08-04:
describía un repo con un módulo (SPIJ) y `pj` como siguiente paso, cuando el
tablero ya contaba 33 de 44 fuentes. `CLAUDE.md` llevaba semanas avisando de que
esta sección «es histórica y puede estar vieja» — un aviso es un parche, no un
arreglo. El texto retirado sigue en el historial de git si alguien lo necesita.

Lo único de aquella sección que no era estado sino **regla**, y por eso sobrevive:
`state/` es el mecanismo oficial anti-duplicados (decisión A1) — no borrarlo y
