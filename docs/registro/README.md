...
| 2026-08-20 | [DUPLICADO_POR_URL.md](2026-08-20/DUPLICADO_POR_URL.md) | La identidad del documento se deriva de `source_url` —decisión correcta para no duplicar al reingestar— pero no cubre el caso inverso: gob.pe publica `005-2023-SUNAFIL-TFL` bajo **dos URL distintas** y entra dos veces. Detectado desde el service al montar un golden set sobre 24 documentos de TFL. Cuesta en recuperación (dos huecos del `top_k` para la misma resolución), en coste de embeddings y en citas. | DU-1 🔴 medir cuántos hay antes de decidir · DU-2 🟡 política de identidad, a decisión del owner | 2026-08-20 · `main` + ingesta real de TFL y consulta a PostgreSQL |
| 2026-08-25 | [INGESTA_SIN_CREDENCIAL.md](2026-08-25/INGESTA_SIN_CREDENCIAL.md) | El despliegue del VPS vio `PUT /points` y `POST /points/delete` **sin credencial y por el dominio público** contra el corpus legal de producción, en curso (23 713 → 30 644 puntos durante la sesión). Se identifica que el cliente es **este repo** por la versión (`qdrant-js 1.17.0`, la que declara su `package.json`), y aparece por qué la fila V-1 llevaba un mes sin cerrarse: cerrar el Qdrant habría roto la ingesta, porque el cliente se construía sin `apiKey` y sin variable donde ponerla. | ✅ `QDRANT_API_KEY` añadida (opcional) · 🔴 IC-1: el corpus sigue abierto, se cierra en el VPS · IC-2 e IC-4 abiertos | 2026-08-25 · `faf490e` · versión del cliente cotejada en los 4 repos + lectura del constructor |
